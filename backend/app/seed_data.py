...
def _ensure_migrations_applied():
    """
    Refuses to seed against a database whose schema doesn't match the code
    yet. This replaces the old Base.metadata.create_all() call, which
    silently did nothing on an existing database missing newer columns —
    that's exactly what caused the benefit-formula fields to go missing.
    """
    ...
    if current_rev != head_rev:
        raise SystemExit(
            f"\nDatabase schema is out of date (at revision {current_rev!r}, "
            f"code expects {head_rev!r}).\nRun this first, then re-run this script:\n\n"
