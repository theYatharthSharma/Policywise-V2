...
class DocumentChunk(Base):
    """
    A chunk of an ingested LIC brochure/document, with its embedding vector.
    Populated by app/rag/ingest.py. Queried by app/rag/retrieve.py at chat
    time to ground the assistant's answers in real brochure text (RAG).
    """
    __tablename__ = "document_chunks"

    id: Mapped[str] = mapped_column(String, primary_key=True, default=gen_uuid)
    policy_id: Mapped[str | None] = mapped_column(String, ForeignKey("policies.id"), nullable=True)
    title: Mapped[str] = mapped_column(String, nullable=False)  # brochure/document title
