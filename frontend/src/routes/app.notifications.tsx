...
function NotificationsPage() {
  const { data = [], isError, refetch } = useQuery({ queryKey: ["notifs"], queryFn: notificationService.list });
  ...
  return (
    <div className="space-y-6">
      ...
      {isError ? (
        <ConnectionErrorState onRetry={() => refetch()} title="Couldn't load notifications" />
      ) : filtered.length === 0 ? (
