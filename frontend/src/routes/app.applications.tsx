...
function ApplicationsPage() {
  const { data: apps = [], isError, refetch } = useQuery({ queryKey: ["apps"], queryFn: applicationService.list });
  ...
  return (
    <div className="space-y-6">
      ...
      {isError ? (
        <ConnectionErrorState onRetry={() => refetch()} title="Couldn't load applications" />
      ) : filtered.length === 0 ? (
