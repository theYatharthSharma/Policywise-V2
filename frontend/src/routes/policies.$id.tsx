...
function PolicyDetails() {
  ...
  const { data: policy, isLoading, isError, refetch } = useQuery({ queryKey: ["policy", id], queryFn: () => policyService.get(id) });
  ...
  if (isError) return (
    <div className="flex min-h-screen flex-col">
      ...
      <div className="mx-auto max-w-3xl px-4 py-20">
        <ConnectionErrorState onRetry={() => refetch()} title="Couldn't load this policy" />
      </div>
    </div>
  );
  ...
  if (!policy) return (
    <div className="flex min-h-screen flex-col">
      ...
      <div className="mx-auto max-w-3xl px-4 py-20 text-center">
        ...
        <p className="mt-2 text-muted-foreground">This policy doesn't exist or may have been withdrawn.</p>
        <Button asChild className="mt-6"><Link to="/policies">Browse all policies</Link></Button>
      </div>
    </div>
  );
