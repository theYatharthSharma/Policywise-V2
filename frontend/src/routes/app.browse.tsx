...
function BrowsePage() {
  ...
  const { data = [], isLoading, isError, refetch } = useQuery({ queryKey: ["policies", "app", q, cat], queryFn: () => policyService.search(q, { category: cat }) });
  ...
  return (
    <div className="space-y-6">
      ...
      {isError ? (
        <ConnectionErrorState onRetry={() => refetch()} title="Couldn't load policies" />
