...
function Dashboard() {
  const { user } = useAuth();
  const { data: recommended = [], isError: recError, refetch: refetchRec } = useQuery({ queryKey: ["policies", "featured"], queryFn: policyService.featured });
  const { data: apps = [], isError: appsError, refetch: refetchApps } = useQuery({ queryKey: ["apps"], queryFn: applicationService.list });
  const { data: notifs = [], isError: notifsError, refetch: refetchNotifs } = useQuery({ queryKey: ["notifs"], queryFn: notificationService.list });
  const anyError = recError || appsError || notifsError;
  const retryAll = () => { refetchRec(); refetchApps(); refetchNotifs(); };
    ...
    <div className="space-y-8">
      {anyError && (
        <ConnectionErrorState
          onRetry={retryAll}
          title="Some dashboard data couldn't load"
          description="We couldn't reach the server for part of this page. The rest still works — try again to refresh everything."
        />
      )}
